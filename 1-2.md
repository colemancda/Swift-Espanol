...
Después de ejecutar el código dentro del caso que emparejo, el programa sale (*exits*) del sentencia de interrupción. Ejecución no continua al siguiente caso, entonces no hay necesidad del explícitamente cesar (*break*) el interruptor que esta al final del código de cada caso.

Puedes usar ```for```-```in``` para iterar sobre elementos en un diccionario proveyendo una pareja de nombres que se usara para cada pareja de llave y valor (*key-value* en inglés). Diccionarios son una colección sin orden, entonces sus llaves y valores son iteradas en una orden arbitraria.
...
> #### Experimento
> Añade otra variable para registrar que tipo de número era el mas grande, así también como que era ese número grande.

Usa ```while``` para repetir un bloque de código hasta que una condicion cambie. La condición de un ciclo puede ser al final también, asegurando que la ciclo se ejecute una vez a lo mínimo.
