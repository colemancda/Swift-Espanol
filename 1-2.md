...
Ademas de propiedades simples que son guardados, propiedades pueden tener métodos de acceso (`get`) y modificación (`set`).
...
En el método de acceso para `perimetro`, el nuevo valor tiene el nombre implícita de ```newValue```. Puedes proveer un nombre explicita en los paréntesis después de ```set```.