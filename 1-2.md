...
Después de ejecutar el código dentro del caso que emparejo, el programa sale (*exits*) del sentencia de interrupción. Ejecución no continua al siguiente caso, entonces no hay necesidad del explícitamente cesar (con `break`) cada caso.
...
Usa `while` para repetir un bloque de código hasta que una condición cambie. La condición de un ciclo puede ser al final también, asegurando que el ciclo se ejecute una vez a lo mínimo.
...
Puedes guardar una indice en una ciclo, usando `..<` para hacer un rango de indices, o mediante inicialización explicita, condición, e incremento. Estas dos ciclos hacen lo mismo:
